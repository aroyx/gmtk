...
func _ready() -> void:
	match gRooms.out_pos:
		gRooms.OutPos.START:
			$Node2D/Player.global_position = $Markers/Start.position
		gRooms.OutPos.STAFF:
			$Node2D/Player.global_position = $Markers/StaffRoom.position
		gRooms.OutPos.HALL_TOP:
			$Node2D/Player.global_position = $Markers/HallTop.position
		gRooms.OutPos.HALL_RIGHT:
			$Node2D/Player.global_position = $Markers/HallRight.position
		gRooms.OutPos.CLASS:
			$Node2D/Player.global_position = $Markers/Class.position
	if gRooms.story_state == gRooms.StoryState.INIT:
			$Node2D/Player.start_timer_to_death(30)
			call_deferred("do_starting_stuff")
	elif gRooms.story_state == gRooms.StoryState.GO_HOME:
		$Node2D/Player.start_timer_to_death(20)
...
func do_starting_stuff():
	$Node2D/Player.playerSay(starting_text)
...
func do_gohome_stuff():
	$Node2D/Player.playerSay(go_home_etxt)
...
func do_next_day_stuff():
	$Node2D/Player.playerSay(next_day_text)
